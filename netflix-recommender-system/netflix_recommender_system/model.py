...
class SupervisedLearning:
    ...
    def predict(self, prediction_df: pd.DataFrame) -> np.ndarray:
        """Pass."""
        numeric_features_prediction = prediction_df[self.numeric_features_names].to_numpy()
        categorical_features_prediction = prediction_df[self.categorical_features_names].to_numpy()
        encoded_categorical_features_prediction = self.encoder.transform(categorical_features_prediction).toarray()
        X_prediction = np.concatenate((numeric_features_prediction, encoded_categorical_features_prediction), axis = 1)
